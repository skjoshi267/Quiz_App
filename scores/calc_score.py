    ...
    if not len(files):
        ...
        for file in files:
            ...
            if len(file_data) > 0:
                ...
                new_data["Category"] = file.replace("./scores\\","").replace(".xlsx","").replace("Fastest Finger First","")
...
def calc_preferences():
    data = pd.read_excel('score.xlsx',engine='openpyxl',sheet_name='Sheet2')
    ...
    result["Name"] = result["Name"].astype(str).str.strip().str.lower()
...
def main():
    ...
    if scores_data.empty:
        ...
        if pref_data.empty:
            ...
            combined_data = pd.merge(scores_data, pref_data, left_on=["Player","Category"], right_on=["Name","category"], how="left")
